...
function getListComboboxCategory($parid=0, $level=0, $childs=array(), $selected_id=0){
	...
	while($rows=$objdata->Fetch_Assoc()){
		...
		$nextlevel=$level+1;
		getListComboboxCategory($id,$nextlevel, $childs, $selected_id);
	}
}
?>
<div class="form-group">
	<label>Chuyên mục tin tức</label>
	<select onchange="change_categories(this)" name="cbo_selected" class="form-control">
		<option value="0">-- Chọn một --</option>
		<?php getListComboboxCategory(0,0, array(), $selected_id);?>
	</select>
</div>
<div class="form-group">
	<label>Link chuyên mục tin tức<span id="err_link" class="mes-error"></span></label><small class="cred"> (*)</small>
