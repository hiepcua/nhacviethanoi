...
function getListComboboxGproduct($parid=0, $level=0, $childs=array(), $selected_id=0){
	...
	while($rows=$objdata->Fetch_Assoc()){
		...
		$nextlevel=$level+1;
		getListComboboxGproduct($id,$nextlevel, $childs, $selected_id);
	}
}
?>
<div class="form-group">
	<label>Nhóm sản phẩm</label>
	<select onchange="change_gproduct(this)" name="cbo_selected" class="form-control">
		<option value="0">-- Chọn một --</option>
		<?php getListComboboxGproduct(0,0, array(), $selected_id);?>
	</select>
</div>
<div class="form-group">
	<label>Link nhóm sản phẩm<span id="err_link" class="mes-error"></span></label><small class="cred"> (*)</small>
	<input name="txtlink" type="text" id="txtlink" class="form-control" value="<?php echo $_link; ?>" placeholder="http://" required />
</div>
<script type="text/javascript">
	function change_gproduct(e){
