...
$res_Cate = SysGetList('tbl_product_group', array(), ' AND id='. $GetID);
if(count($res_Cate) <= 0){
...
$arr_childs = array();
$res_children = SysGetList('tbl_product_group', array('id'), " AND `path` LIKE '".$row['path']."%'");
	...
	<div class="row">
		<div class="col-md-8">
			<div class="form-group">
				<label>Tên nhóm sản phẩm </label><font color="red">*</font>
				<input type="text" id="txt_name" name="txt_name" class="form-control required" value="<?php echo $row['title'];?>" placeholder="Tên nhóm sản phẩm">
			</div>

			<div class="form-group">
				<label>Nhóm cha</label>
				<select class="form-control" name="cbo_par" id="ajax_cbo_par">
...
<script type="text/javascript">
	$(document).ready(function(){
		$('#ajax_cbo_par').select2();
		$("form#frm_action").submit(function(e) {
			if(validForm()){
				e.preventDefault();    
				var formData = new FormData(this);
				var _url="<?php echo ROOTHOST;?>ajaxs/product_group/process_edit.php";
				$.ajax({
					url: _url,
					type: 'POST',
					data: formData,
