...
$get_gproduct = isset($_GET['gproduct']) ? antiData($_GET['gproduct']) : 'all';
...
if($get_gproduct !== '' && $get_gproduct !== 'all' ){
	$strWhere.=" AND `id_pgroup` = '".$get_gproduct."' ";
}

/*Begin pagging*/
$total_rows = SysCount('tbl_product_type',$strWhere);
$max_rows = 20;
			...
			<div class="col-md-10">
				<form id="frm_search" method="get" action="<?php echo ROOTHOST.COMS;?>">
					<div class="frm-search-box form-inline pull-left">
						<input type='text' id='txt_title' name='q' class='form-control' value="<?php echo $get_q?>" placeholder="Tên loại sản phẩm..." />
						&nbsp&nbsp&nbsp
						...
						&nbsp&nbsp&nbsp Nhóm sản phẩm &nbsp
						<select name="gproduct" class="form-control" id="gproduct">
							<option value="all">Tất cả</option>
							<?php getListComboboxCategories();?>
							<script language="javascript">
								$(document).ready(function(){
									cbo_Selected('gproduct','<?php echo $get_gproduct;?>');
								});
							</script>
						</select>
						&nbsp&nbsp&nbsp
...
<script type="text/javascript">
	...
	function add(){
		var _url="<?php echo ROOTHOST;?>ajaxs/product_type/form_add.php";
		$.get(_url, function(req){
			$('#popup_modal .modal-dialog').addClass('modal-xl');
			$('#popup_modal .modal-title').text('Thêm mới loại sản phẩm');
	...
	function getTable(strwhere, start, max_rows){
		var _url="<?php echo ROOTHOST;?>ajaxs/product_type/get_table.php";
		var _data={
			"strwhere": strwhere,
			"start": start,
			"max_rows": max_rows,
	...
	function edit(id){
		if(parseInt(id)!==0){
			var _url="<?php echo ROOTHOST;?>ajaxs/product_type/form_edit.php";
			$.get(_url, {"id": id}, function(req){
				$('#popup_modal .modal-dialog').addClass('modal-xl');
				$('#popup_modal .modal-title').text('Cập nhật loại sản phẩm');
				$('#popup_modal .modal-body').html(req);
				$('#popup_modal').modal('show');
			});
		}
	...
	function active(e){
		...
		$.post('<?php echo ROOTHOST;?>ajaxs/product_type/active.php', {"id": id}, function(res){
			if(parseInt(res)==1){
	...
	function order(e){
		...
		$.post('<?php echo ROOTHOST;?>ajaxs/product_type/order.php', _data, function(res){
			if(parseInt(res)==1){
				window.location.reload();
			}else if(parseInt(res)==3){
				alert('Bạn không có quyền thực hiện chức năng này!');
	...
	function delete1(e){
		...
		if(confirm('Bạn có chắc muốn xóa?')){
			$.post('<?php echo ROOTHOST;?>ajaxs/product_type/delete.php', {"id": id}, function(res){
				if(parseInt(res)==1){
					window.location.reload();
				}else if(parseInt(res)==3){
					alert('Bạn không có quyền thực hiện chức năng này!');
