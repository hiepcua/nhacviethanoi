...
function paging2($total_rows,$max_rows,$cur_page){
	$max_pages=ceil($total_rows/$max_rows);
	$start=$cur_page-5; if($start<1)$start=1;
	$end=$cur_page+5;   if($end>$max_pages)$end=$max_pages;
	$paging='
...
function postCurentPage($max_pages=1){
	if($max_pages==0) $max_pages=1;
	$cur_page=isset($_POST['txtCurnpage'])?antiData($_POST['txtCurnpage'],'int'):1;
	if($cur_page<1)$cur_page=1;
	if($cur_page>$max_pages) $cur_page=$max_pages;
	return $cur_page;
}
...
function Curl_Post($url,$jsonBody){
	...
	curl_setopt($curl, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($jsonBody)));  
	$resp = curl_exec($curl);
	curl_close($curl);
