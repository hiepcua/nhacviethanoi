...
class CLS_UPLOAD{
	...
	var $file_size=NULL;
	var $file_error=NULL;
	var $array_type=array('application/vnd.ms-excel','text/xls','text/xlsx','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
	...
	function checkType(){
		if(in_array($this->file_type,$this->array_type))
			...
			die('File nguồn không tồn tại hoặc không phải định dạng cho phép !. '.$this->file_type);
	}
	...
	function checkSize(){
		if($this->file_size < $this->max_size)
			...
			die('File nguồn không quá lớn so với kích thước cho phép!. '.$this->file_size);
	}
	...
	function checkError(){
		if($this->file_error==0)
			...
			die('Có lỗi trong quá trình truyền file!. '.$this->file_error);
	}
